...
use MSFSI\Helper\Factory\ResponseDecisionHandler;
...
final class SendResponseHandler extends BaseHandler
{
	...
	{
		if(MSFSI_DEBUG) MoIDPUtility::mo_debug("Generating Login Response");
		...
		if(MSFSI_DEBUG) MoIDPUtility::mo_debug( "Login Response generated: " . $response);
	...
	public function getSAMLResponseParams($args,$current_user)
	{
		...
		$issuer 	= get_site_option('mo_idp_entity_id') ?  sanitize_text_field(get_site_option('mo_idp_entity_id')) : MSFSI_URL;
	...
	{
		if(MSFSI_DEBUG) MoIDPUtility::mo_debug("Sending SAML Login Response");
	...
	{
		if(MSFSI_DEBUG) MoIDPUtility::mo_debug("Sending WS-FED Login Response");
        echo'
		<html>
			<head>
				<meta http-equiv="cache-control" content="no-cache">
