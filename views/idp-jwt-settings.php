<?php
	...
	echo		'<div class="mo_idp_premium_option_text">
                    ...
                    <a href="'.esc_url($license_url).'">Licensing Tab</a> to learn more.
                </div>
                <div class="mo_premium_option">
                    <form name="f" method="post" action="'.esc_url($post_url).'">';
        echo			esc_attr($sp_exists) ? '<input type="hidden" name="option" value="mo_add_idp" />' : '<input type="hidden" name="option" value="mo_edit_idp" />';
        echo			'<input type="hidden" name="service_provider" value="'.(isset($sp) && !empty($sp) ? esc_attr($sp->id) : "").'" />
                    ...
                    <input  type="checkbox"  
                            '.(esc_attr(!$registered) || empty($sp) ? 'disabled title="Disabled. Configure your Identity Provider"' : '').' 
                            onchange="window.location=\''.esc_url($sp_page_url).'\'" />
