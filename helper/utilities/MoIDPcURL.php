...
class MoIDPcURL
{
    ...
    public static function submit_contact_us( $q_email, $q_phone, $query )
    {
        ...
        $query  = '['."WP Freshdesk Integration Plugin"." - " . MSFSI_VERSION .']: ' . $query;
        ...
        $fields = array(
                    'firstName'	=> sanitize_text_field($current_user->user_firstname),
                    'lastName'	=> sanitize_text_field($current_user->user_lastname),
                    'company' 	=> sanitize_text_field($_SERVER['SERVER_NAME']),
                    'email' 	=> sanitize_email($q_email),
    ...
    private static function callAPI($url, $json_string, $headers = ["Content-Type" => "application/json"])
    {
        $args = [
            ...
            'blocking'      => true,
            'headers'       => $headers,
            'sslverify'     => MSFSI_TEST ? false: true,
        ];
